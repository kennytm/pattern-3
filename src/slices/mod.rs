...
impl<'h, T: 'h> Haystack for &'h [T] {
    ...
    #[inline]
    unsafe fn split_around_unchecked(self, start: usize, end: usize) -> (Self, Self, Self) {
        let st = self.as_ptr();
        let c1 = st.add(start);
        let c2 = st.add(end);
        (
            from_raw_parts(st, start),
            from_raw_parts(c1, end - start),
            from_raw_parts(c2, self.len() - end),
        )
    }
...
impl<'h, T: 'h> Haystack for &'h mut [T] {
    ...
    unsafe fn split_around_unchecked(self, start: usize, end: usize) -> (Self, Self, Self) {
        ...
        let c2 = st.add(end);
        (
            from_raw_parts_mut(st, start),
            from_raw_parts_mut(c1, end - start),
            from_raw_parts_mut(c2, self.len() - end),
        )
    }
...
impl<T> Haystack for Vec<T> {
    ...
    #[inline]
    unsafe fn split_around_unchecked(mut self, start: usize, end: usize) -> (Self, Self, Self) {
